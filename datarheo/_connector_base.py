...
from datarheo._util.telemetry import (
    EventState,
...
class ConnectorBase(abc.ABC):
    ...
    def get_config(self) -> dict[str, Any]:
        ...
        if self._config_dict is None:
            raise exc.DataRheoConnectorConfigurationMissingError(
    ...
    def validate_config(self, config: dict[str, Any] | None = None) -> None:
        ...
        try:
            ...
            validation_ex = exc.DataRheoConnectorValidationFailedError(
                connector_name=self.name,
    ...
    def _get_spec(self, *, force_refresh: bool = False) -> ConnectorSpecification:
        ...
        * Make sure the subprocess is killed when the function returns.
        ...
        Raises:
            DataRheoConnectorSpecFailedError: If the spec operation fails.
            DataRheoConnectorMissingSpecError: If the spec operation does not return a spec.
        """
        if force_refresh or self._spec is None:
            ...
            except exc.DataRheoSubprocessError as ex:
                raise exc.DataRheoConnectorSpecFailedError(
                    connector_name=self.name,
    ...
    def check(self) -> None:
        ...
        * Make sure the subprocess is killed when the function returns.
        ...
        with as_temp_files([self._hydrated_config]) as [config_file]:
            try:
                for msg in self._execute(["check", "--config", config_file]):
                    if msg.type == Type.CONNECTION_STATUS and msg.connectionStatus:
                        ...
                        sleep(1)
                        raise exc.DataRheoConnectorCheckFailedError(
                            connector_name=self.name,
            ...
            except exc.DataRheoConnectorFailedError as ex:
                raise exc.DataRheoConnectorCheckFailedError(
                    connector_name=self.name,
    ...
    def _execute(
        ...
        * Read the output line by line of the subprocess and serialize them AirbyteMessage objects.
        ...
        Raises:
            DataRheoConnectorFailedError: If the process returns a failure status (non-zero).
        """
        ...
        try:
            ...
            for line in self.executor.execute(args, stdin=stdin, suppress_stderr=suppress_stderr):
                try:
                    ...
                    self._peek_datarheo_message(message)
                    yield message
        ...
        except exc.DataRheoSubprocessFailedError as ex:
