...
from datarheo.agents.models import AgentWorkspaceInfo
...
class AgentOrganization:
    ...
    def from_env(cls, organization_id: str | None = None) -> AgentOrganization:
        """Create an `AgentOrganization` from the `DATARHEO_CLOUD_*` environment variables."""
    ...
    def as_cloud_organization(self) -> CloudOrganization:
        """Return this organization as an `datarheo.cloud.CloudOrganization`.

        Every Agents organization is also a Cloud organization, so this conversion needs no
        API call. It requires a known organization ID, and raises `DataRheoInputError`
        ...
        if not self.organization_id:
            raise DataRheoInputError(
                ...
                guidance=(
                    "Provide `organization_id`, or set the `DATARHEO_CLOUD_ORGANIZATION_ID` "
