...
"""Actions PyDataRheo rejects before sending them to the Agents API.

`download` returns a binary stream rather than JSON, and PyDataRheo does not yet support
...
"""Pagination conveniences PyDataRheo merges into the connector's `params`.

Maps the PyDataRheo argument name to the connector's own `params` key: the Agents API calls
page size `limit`, which PyDataRheo does not expose under that name because `limit` reads as
...
class AgentConnector:
    ...
    def execute(  # noqa: PLR0913  # Explicit args are the point of this public API.
        ...
        *,
        ...
        example `{"repository": "Vrahad-Analytics/pydatarheo"}`. All other arguments are interpreted
        by PyDataRheo or by the Agents API itself:
        ...
        The `download` action is rejected, because it returns a binary stream and PyDataRheo
        does not yet support streaming responses.
        """
        if action in UNSUPPORTED_ACTIONS:
            raise DataRheoInputError(
                message=f"The {action!r} action is not supported by PyDataRheo.",
                guidance=(
                    "This action returns a binary stream instead of JSON, and PyDataRheo does "
