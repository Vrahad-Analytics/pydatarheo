...
CONVERSION_MAP = {
    ...
    # Technically 'object' and 'array' as JSON Schema types, not datarheo types.
...
def _get_datarheo_type(  # noqa: PLR0911  # Too many return statements
    json_schema_property_def: dict[str, str | dict | list],
) -> tuple[str, str | None]:
    """Get the datarheo type and subtype from a JSON schema property definition.
    ...
    if json_schema_type == "array":
        ...
        if isinstance(items_def, dict):
            try:
                subtype, _ = _get_datarheo_type(items_def)
    ...
    err_msg = f"Could not determine datarheo type from JSON schema type: {json_schema_property_def}"
...
class SQLTypeConverter:
    ...
    def to_sql_type(  # noqa: PLR0911  # Too many return statements
        ...
        try:
            ...
            print(f"Could not determine datarheo type from JSON schema: {json_schema_property_def}")
        except KeyError:
            # pyrefly: ignore[unbound-name]
            print(f"Could not find SQL type for datarheo type: {airbyte_type}")
        else:
