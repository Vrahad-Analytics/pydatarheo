...
"""MCP (Model Context Protocol) server for PyDataRheo connector management.
...
- **HTTP**: For hosted deployment. Start via `datarheo-mcp-http` entry point or
  `poe mcp-serve-http`. This server maps its own branded `DATARHEO_MCP_*` env vars
  ...
  supports two client shapes on the same deployment:
    - **Interactive** (humans in a browser): Keycloak Authorization Code + PKCE
      via `OIDCProxy`, active once `DATARHEO_MCP_OIDC_CLIENT_ID`,
      `DATARHEO_MCP_OIDC_CLIENT_SECRET`, and `DATARHEO_MCP_OIDC_CONFIG_URL` (the
    ...
    - **Headless** (agents, CI): the client mints its own short-lived bearer
      ...
      `JWTVerifier`, active once a signing-key source (`DATARHEO_MCP_AUTH_JWKS_URI`
      or `DATARHEO_MCP_AUTH_JWT_PUBLIC_KEY`) is configured (no browser, no
...
- `DATARHEO_MCP_SEGMENT_WRITE_KEY`: Segment write key for MCP tool-call telemetry
  across all transports. Defaults to the PyDataRheo application key and is
  ignored when `DO_NOT_TRACK` or `DATARHEO_OFFLINE_MODE` is set.
"""
...
from datarheo._util.meta import set_mcp_mode
from datarheo._util.telemetry import DO_NOT_TRACK, PYDATARHEO_APP_TRACKING_KEY
...
from datarheo.mcp._tool_utils import (
    ...
    datarheo_module_filter,
    datarheo_readonly_mode_filter,
    validate_datarheo_domains,
)
from datarheo.mcp.agents import register_agents_tools
from datarheo.mcp.cloud import register_cloud_tools
from datarheo.mcp.interactive import register_interactive_tools
...
# auth vars use the branded `DATARHEO_MCP_*` namespace as an added layer over
...
OIDC_CLIENT_ID_ENV = "DATARHEO_MCP_OIDC_CLIENT_ID"
OIDC_CLIENT_SECRET_ENV = "DATARHEO_MCP_OIDC_CLIENT_SECRET"
OIDC_CONFIG_URL_ENV = "DATARHEO_MCP_OIDC_CONFIG_URL"
...
DATARHEO_CLOUD_REQUIRED_OIDC_SCOPES: str = "openid email profile"
...
JWKS_URI_ENV = "DATARHEO_MCP_AUTH_JWKS_URI"
JWT_PUBLIC_KEY_ENV = "DATARHEO_MCP_AUTH_JWT_PUBLIC_KEY"
JWT_ISSUER_ENV = "DATARHEO_MCP_AUTH_ISSUER"
JWT_AUDIENCE_ENV = "DATARHEO_MCP_AUTH_AUDIENCE"
JWT_ALGORITHM_ENV = "DATARHEO_MCP_AUTH_ALGORITHM"
...
# config) lives in the deployment's own package, keeping PyDataRheo generic.
OIDC_CLIENT_STORAGE_FACTORY_ENV = "DATARHEO_MCP_OIDC_CLIENT_STORAGE_FACTORY"
...
class _ClientStorageFactory(Protocol):
    ...
    `DATARHEO_MCP_OIDC_CLIENT_STORAGE_FACTORY` (`"package.module:callable"`). The
    ...
    lets PyDataRheo stay generic — the infrastructure-specific factory ships in
...
def _resolve_client_storage(*, encryption_source_material: str) -> AsyncKeyValue | None:
    ...
    Reads `DATARHEO_MCP_OIDC_CLIENT_STORAGE_FACTORY` (`"package.module:callable"`),
    ...
    for single-instance local dev). PyDataRheo stays backend-agnostic: it never
...
def _create_auth() -> AuthProvider | None:
    ...
    Reads this server's branded `DATARHEO_MCP_*` env vars and maps them into the
    ...
    (`DATARHEO_MCP_AUTH_JWKS_URI` or `DATARHEO_MCP_AUTH_JWT_PUBLIC_KEY`) is
    ...
    if oidc_client_id and oidc_client_secret:
        ...
        oidc = OIDCAuthConfig(
            ...
            base_url=base_url,
            required_scopes=DATARHEO_CLOUD_REQUIRED_OIDC_SCOPES.split(),
...
SEGMENT_WRITE_KEY_ENV = "DATARHEO_MCP_SEGMENT_WRITE_KEY"

SEGMENT_USER_ID = "datarheo-mcp"
"""Identifies the PyDataRheo MCP server as the event source.
...
def _segment_write_key() -> str | None:
    ...
    offline_mode_from_env = os.environ.get("DATARHEO_OFFLINE_MODE")
    # Dotenv secrets load after constants are imported, so check the environment at call time.
    offline_mode = DATARHEO_OFFLINE_MODE or _str_to_bool(offline_mode_from_env, default=False)
    if os.environ.get(DO_NOT_TRACK) or offline_mode:
        return None

    return _env_or_default(SEGMENT_WRITE_KEY_ENV, PYDATARHEO_APP_TRACKING_KEY) or None
...
"""The DataRheo MCP Server application instance."""
...
def main() -> None:
    ...
    """
    print("Starting DataRheo MCP server.", file=sys.stderr)
    try:
        asyncio.run(app.run_stdio_async())
    except KeyboardInterrupt:
        print("DataRheo MCP server interrupted by user.", file=sys.stderr)
    except Exception as ex:
        print(f"Error running DataRheo MCP server: {ex}", file=sys.stderr)
        sys.exit(1)

    print("DataRheo MCP server stopped.", file=sys.stderr)
