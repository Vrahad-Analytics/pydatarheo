...
"""HTTP transport entry point for the DataRheo MCP server.
...
this server's branded `DATARHEO_MCP_*` env vars into the typed configs consumed
...
- `DATARHEO_MCP_OIDC_CLIENT_ID`: OIDC client identifier
- `DATARHEO_MCP_OIDC_CLIENT_SECRET`: OIDC client secret
...
- `DATARHEO_MCP_AUTH_JWKS_URI`: JWKS endpoint used to verify token signatures
- `DATARHEO_MCP_AUTH_JWT_PUBLIC_KEY`: static public key (alternative to the JWKS
  URI)
- `DATARHEO_MCP_AUTH_ISSUER`: expected token issuer
- `DATARHEO_MCP_AUTH_AUDIENCE`: expected token audience
- `DATARHEO_MCP_AUTH_ALGORITHM`: signing algorithm override

Opt-in static client credentials:

- `DATARHEO_MCP_AUTH_ALLOW_CLIENT_CREDENTIALS`: enable `Client-Id` /
  `Client-Secret` headers and HTTP Basic credentials. This is an exchange-and-
  rewrite layer, not a bearer-token verifier; configure `DATARHEO_MCP_AUTH_JWKS_URI`
...
from datarheo.mcp._client_credentials import (
...
from datarheo.mcp.server import (
    DEFAULT_HTTP_HOST,
    DEFAULT_HTTP_PORT,
...
MCP_LANDING_TITLE = "DataRheo MCP Server"
MCP_LANDING_DOCS_URL = "https://docs.airbyte.com/ai-agents/"
RELEASE_TAG_URL_TEMPLATE = "https://github.com/Vrahad-Analytics/pydatarheo/releases/tag/v{}"
COMMIT_URL_TEMPLATE = "https://github.com/Vrahad-Analytics/pydatarheo/commit/{}"
RELEASES_URL = "https://github.com/Vrahad-Analytics/pydatarheo/releases"
_FINAL_VERSION_PATTERN = re.compile(r"\d+(?:\.\d+)*")
_COMMIT_SHA_PATTERN = re.compile(r"[0-9a-f]{7,40}")


def _landing_version_str() -> str:
    """Return the installed PyDataRheo version for the landing-page footer."""
...
def _log_auth_status() -> None:
    ...
    if app.auth is None and client_credentials_enabled():
        logger.warning(
            ...
            "are not checked. Set `DATARHEO_MCP_AUTH_JWKS_URI` or "
            "`DATARHEO_MCP_AUTH_JWT_PUBLIC_KEY` to require bearer verification."
        )
        ...
        logger.warning(
            ...
            "is unauthenticated. Set `DATARHEO_MCP_OIDC_CLIENT_ID`/"
            "`DATARHEO_MCP_OIDC_CLIENT_SECRET`/`DATARHEO_MCP_OIDC_CONFIG_URL` "
            "(interactive) or `DATARHEO_MCP_AUTH_JWKS_URI`/"
            "`DATARHEO_MCP_AUTH_JWT_PUBLIC_KEY` (headless) to require auth."
        )
    else:
...
def main() -> None:
    """Start the DataRheo MCP server with HTTP transport."""
    ...
    logger.info(
        "Starting DataRheo MCP HTTP server on %s:%d (mcp_path=%r)",
        http_host,
        DEFAULT_HTTP_PORT,
        mcp_path,
    )
    ...
    try:
        ...
        logger.info("DataRheo MCP HTTP server interrupted by user.")
