...
"""Trusted-execution guards for DataRheo MCP backend helpers.
...
resolution. It is controlled solely by the `DATARHEO_MCP_TRUSTED_EXECUTION` server
environment variable (`datarheo.constants.MCP_TRUSTED_EXECUTION_ENV_VAR`) and defaults to
...
def is_trusted_execution_enabled() -> bool:
    ...
    Reads `DATARHEO_MCP_TRUSTED_EXECUTION` from the server environment only. A value of
...
def raise_if_untrusted_execution_context(feature: str) -> None:
    ...
    resolution). It raises `datarheo.exceptions.DataRheoTrustedExecutionRequiredError`
    when the gate is off, independently of whether the corresponding tool was hidden from
    the listing.
    """
    if not is_trusted_execution_enabled():
        raise DataRheoTrustedExecutionRequiredError(feature=feature)
