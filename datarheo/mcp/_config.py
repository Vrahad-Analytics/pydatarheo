...
from datarheo._util.meta import is_interactive
from datarheo.mcp._guards import is_trusted_execution_enabled
...
def load_secrets_to_env_vars() -> None:
    ...
    When trusted execution is disabled (`datarheo.mcp._guards.is_trusted_execution_enabled`),
    ...
    `DATARHEO_MCP_ENV_FILE`) and are never caller-controllable, and the function-layer guards
    in `datarheo.mcp._arg_resolvers.resolve_connector_config` are what actually block an
    ...
    if DATARHEO_MCP_DOTENV_PATH_ENVVAR in os.environ:
        dotenv_path = Path(os.environ[DATARHEO_MCP_DOTENV_PATH_ENVVAR]).absolute()
