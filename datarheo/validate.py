...
"""Defines the `datarheo-validate-source` CLI.

This tool checks if connectors are compatible with PyDataRheo.
"""
...
from datarheo.constants import NO_UV
...
def _run_subprocess_and_raise_on_failure(args: list[str]) -> None:
    ...
    if result.returncode != 0:
        raise exc.DataRheoSubprocessFailedError(
            run_args=args,
...
def install_only_test(connector_name: str) -> None:
    ...
    source = dr.get_source(connector_name)
    source._get_spec(force_refresh=True)  # noqa: SLF001  # Member is private until we have a public API for it.


def run() -> None:
    """Handle CLI entrypoint for the `datarheo-validate-source` command.

    It's called like this:
    > datarheo-validate-source —connector-dir . -—sample-config secrets/config.json
...
def validate(connector_dir: str, sample_config: str, *, validate_install_only: bool) -> None:
    ...
    registry = {
        "sources": [
            {
                "dockerRepository": f"datarheo/{connector_name}",
                "dockerImageTag": "0.0.1",
                "remoteRegistries": {
                    "pypi": {"packageName": f"datarheo-{connector_name}", "enabled": True}
                },
            },
        ],
    }
    ...
    with tempfile.NamedTemporaryFile(
        ...
        temp_file.flush()
        os.environ["DATARHEO_LOCAL_REGISTRY"] = str(temp_file.name)
        try:
            ...
            del os.environ["DATARHEO_LOCAL_REGISTRY"]
            temp_file.close()
