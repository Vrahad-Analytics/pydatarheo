...
if TYPE_CHECKING:
    ...
    from datarheo._writers.jsonl import FileWriterBase
...
class SqlConfig(BaseModel, abc.ABC):
    ...
    schema_name: str = Field(default="datarheo_raw")
...
class SqlProcessorBase(abc.ABC):
    ...
    def catalog_provider(
        ...
        Raises:
            DataRheoInternalError: If the catalog manager is not set.
        """
    ...
    def state_writer(
        ...
        Raises:
            DataRheoInternalError: If the state manager is not set.
        """
    ...
    def process_datarheo_messages(
        ...
        *,
        ...
        if not isinstance(write_strategy, WriteStrategy):
            raise exc.DataRheoInternalError(  # pyrefly: ignore[missing-attribute]
    ...
    def _get_sql_column_definitions(
        ...
        columns[DR_RAW_ID_COLUMN] = self.type_converter_class.get_string_type()
        columns[DR_EXTRACTED_AT_COLUMN] = sqlalchemy.TIMESTAMP(timezone=True)
        columns[DR_META_COLUMN] = self.type_converter_class.get_json_type()
    ...
    def _swap_temp_table_with_final_table(
        ...
        if final_table_name is None:
            raise exc.DataRheoInternalError(message="Arg 'final_table_name' cannot be None.")
        if temp_table_name is None:
            raise exc.DataRheoInternalError(message="Arg 'temp_table_name' cannot be None.")
