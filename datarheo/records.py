...
1. PyDataRheo normalizes top-level record keys to lowercase, replacing spaces and hyphens with
   underscores.
2. PyDataRheo does not normalize nested keys on sub-properties.
...
Similar to column handling, PyDataRheo normalizes table names to the lowercase version of the stream
name and may remove or normalize special characters.

### DataRheo-Managed Metadata Columns

PyDataRheo adds the following columns to every record:
...
The names of these columns are included in the `datarheo.constants` module for programmatic
reference.

## Schema Evolution

PyDataRheo supports a very basic form of schema evolution:
...
from datarheo._util.name_normalizers import LowerCaseNormalizer, NameNormalizerBase
...
class StreamRecordHandler:
    ...
    def __init__(
        ...
        *,
        ...
        for internal_col in DR_INTERNAL_COLUMNS:
    ...
    def __init__(
        ...
        *,
        ...
        if with_internal_columns:
            self.update(
                {
                    DR_RAW_ID_COLUMN: uuid7str(),
                    DR_EXTRACTED_AT_COLUMN: extracted_at or datetime.now(timezone.utc),
                    DR_META_COLUMN: {},
                }
            )
