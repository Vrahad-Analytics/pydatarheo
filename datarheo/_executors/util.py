...
from datarheo._executors.docker import DEFAULT_CONTAINER_TEMP_DIR, DockerExecutor
from datarheo._executors.local import PathExecutor
from datarheo._executors.noop import NoOpExecutor
from datarheo._executors.python import VenvExecutor
from datarheo._util.meta import which
from datarheo._util.telemetry import EventState, log_install_state  # Non-public API
from datarheo.constants import DATARHEO_OFFLINE_MODE, DEFAULT_PROJECT_DIR, TEMP_DIR_OVERRIDE
...
def _try_get_manifest_connector_files(
    ...
    Raises:
        - `DataRheoInputError`: If `source_name` is `None`.
        - `DataRheoConnectorInstallationError`: If the manifest cannot be downloaded or parsed,
...
def get_connector_executor(  # noqa: PLR0912, PLR0913, PLR0914, PLR0915, C901 # Too complex
    ...
    *,
    ...
    For documentation of each arg, see the function `datarheo.sources.util.get_source()`.
    """
    ...
    try:
    ...
    except exc.DataRheoConnectorNotRegisteredError as ex:
        ...
        if not DATARHEO_OFFLINE_MODE:
            # If the user has not enabled offline mode, raise an error.
            raise exc.DataRheoConnectorRegistryError(
    ...
    if docker_image:
        ...
        container_temp_dir = DEFAULT_CONTAINER_TEMP_DIR
