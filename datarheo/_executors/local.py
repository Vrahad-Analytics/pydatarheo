...
class PathExecutor(Executor):
    ...
    def install(self) -> NoReturn:
        raise exc.DataRheoConnectorInstallationError(
            message="Connector cannot be installed because it is not managed by PyDataRheo.",
            connector_name=self.name,
        )

    def uninstall(self) -> NoReturn:
        raise exc.DataRheoConnectorInstallationError(
            message="Connector cannot be uninstalled because it is not managed by PyDataRheo.",
            connector_name=self.name,
        )
