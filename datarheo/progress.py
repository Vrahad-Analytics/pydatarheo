...
use Rich, and Rich only supports one live view at a time. PyDataRheo will try to use smart defaults
...
Logging is controlled by the `DATARHEO_LOGGING_ROOT` and `DATARHEO_STRUCTURED_LOGGING` environment
variables, as described in `datarheo.logs`. If `DATARHEO_STRUCTURED_LOGGING` is set, logs will be
...
class ProgressTracker:  # noqa: PLR0904  # Too many public methods
    ...
    def reset_progress_style(
        ...
        ```python
        from datarheo.progress import progress, ProgressStyle
