...
The PyDataRheo CLI provides a command-line interface for testing connectors and running benchmarks.

After installing PyDataRheo, the CLI can be invoked with the `pydatarheo` CLI executable, or the
shorter `pydr` alias.
...
You can also use `pipx` or the fast and powerful `uv` tool to run the PyDataRheo CLI
...
# Run the PyDataRheo CLI using `uvx`:
uvx --from=pydatarheo pydr --help
```

Example `benchmark` Usage:

```bash
# PyDataRheo System Benchmark (no-op):
pydr benchmark --num-records=2.4e6

# Source Benchmark:
pydr benchmark --source=source-hardcoded-records --config='{count: 400000}'
pydr benchmark --source=source-hardcoded-records --config='{count: 400000}' --streams='*'
pydr benchmark --source=source-hardcoded-records --config='{count: 4000}' --streams=dummy_fields

# Source Benchmark from Docker Image:
pydr benchmark --source=airbyte/source-hardcoded-records:latest --config='{count: 400_000}'
pydr benchmark --source=airbyte/source-hardcoded-records:dev --config='{count: 400_000}'

# Destination Benchmark:
pydr benchmark --destination=destination-dev-null --config=/path/to/config.json

# Benchmark a Local Python Source (source-s3):
pydr benchmark --source=$(poetry run which source-s3) --config=./secrets/config.json
# Equivalent to:
LOCAL_EXECUTABLE=$(poetry run which source-s3)
CONFIG_PATH=$(realpath ./secrets/config.json)
pydr benchmark --source=$LOCAL_EXECUTABLE --config=$CONFIG_PATH
```

Example `validate` Usage:

```bash
pydr validate --connector=source-hardcoded-records
pydr validate --connector=source-hardcoded-records --config='{count: 400_000}'
```
"""
...
from datarheo._util.destination_smoke_tests import run_destination_smoke_test
from datarheo.destinations.util import get_destination, get_noop_destination
...
in the secret store. By default, PyDataRheo will look for secrets in environment variables and
...
CONFIG_HELP = (
    ...
    "PyDataRheo secrets can be accessed by prefixing the secret name with 'SECRET:'. "
...
cli = App(
    name="pydr",  # pyrefly: ignore[unexpected-keyword]
    help="PyDataRheo CLI.",  # pyrefly: ignore[unexpected-keyword]
...
def validate(
    *,
    connector: Annotated[
        str | None,
        Parameter(help="The connector name or a path to the local executable."),
    ] = None,
    pip_url: Annotated[
        str | None,
        Parameter(
    ...
    config: Annotated[
        str | None,
        Parameter(help=CONFIG_HELP),
    ] = None,
    use_python: Annotated[
        str | None,
        Parameter(help=USE_PYTHON_HELP),
    ] = None,
) -> None:
...
# Append the full CLI guidance so `pydr validate --help` continues to include
# the extended usage notes that Click's `help=` argument previously injected.
validate.__doc__ = (validate.__doc__ or "") + "\n" + CLI_GUIDANCE
...
def benchmark(
    *,
    source: Annotated[
        ...
        Parameter(
            help=(
                ...
                "If the name contains a colon (':'), it will be interpreted as a docker image "
                "and tag. "
            ),
        ),
    ] = None,
    streams: Annotated[
        ...
        Parameter(
            help=(
                "A comma-separated list of stream names to select for reading. If set to '*', "
                "all streams will be selected. Defaults to '*'."
            ),
        ),
    ] = "*",
    ...
    destination: Annotated[
        ...
        Parameter(
            help=(
                ...
                "If a path is provided, it will be interpreted as a path to the local "
                "executable. "
            ),
        ),
    ] = None,
...
def destination_smoke_test(  # noqa: PLR0913
    *,
    ...
    config: Annotated[
        str | None,
        Parameter(help="The destination configuration. " + CONFIG_HELP),
    ] = None,
    pip_url: Annotated[
        str | None,
        Parameter(
    ...
    `pydr destination-smoke-test --destination=destination-dev-null`

    `pydr destination-smoke-test --destination=destination-snowflake
    --config=./secrets/snowflake.json`

    `pydr destination-smoke-test --destination=destination-motherduck
