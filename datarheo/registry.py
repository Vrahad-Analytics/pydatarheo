...
from datarheo._registry_utils import fetch_registry_version_date, parse_changelog_html
from datarheo._util.meta import is_docker_installed
...
_REGISTRY_ENV_VAR = "DATARHEO_LOCAL_REGISTRY"
...
def _is_registry_disabled(url: str) -> bool:
    return url.upper() in {"0", "F", "FALSE"} or DATARHEO_OFFLINE_MODE
...
def get_connector_metadata(name: str) -> ConnectorMetadata | None:
    ...
    if name not in cache:
        raise exc.DataRheoConnectorNotRegisteredError(
            connector_name=name,
            context={
                "registry_url": _get_registry_url(),
...
def get_connector_api_docs_urls(connector_name: str) -> list[ApiDocsUrl]:
    ...
    Raises:
        DataRheoConnectorNotRegisteredError: If the connector is not found in the registry.
    """
