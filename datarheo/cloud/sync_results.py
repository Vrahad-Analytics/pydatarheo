...
determined by inspecting the constant `datarheo.cloud.constants.READABLE_DESTINATION_TYPES`.
...
from datarheo.cloud.constants import FAILED_STATUSES, FINAL_STATUSES
    ...
    **This class is not meant to be instantiated directly.** Instead, obtain a `SyncResult` by
    ...
    def raise_failure_status(
        ...
        *,
        ...
        method will raise a `DataRheoConnectionSyncError`.
        ...
        *,
        ...
        while True:
            ...
            if time.time() - start_time > wait_timeout:
                if raise_timeout:
                    raise DataRheoConnectionSyncTimeoutError(
                        workspace=self.workspace,
                        connection_id=self.connection.connection_id,
                        job_id=self.job_id,
    ...
    def get_dataset(self, stream_name: str) -> CachedDataset:
        """Retrieve an `datarheo.datasets.CachedDataset` object for a given stream name.
    ...
    def streams(
        ...
        """Return a mapping of stream names to `datarheo.CachedDataset` objects.
