...
from datarheo.constants import SECRETS_HYDRATION_PREFIX
from datarheo.secrets.base import SecretManager, SecretSourceEnum, SecretString
from datarheo.secrets.config import _get_secret_sources
...
def is_secret_available(
    ...
    except exc.DataRheoSecretNotFoundError:
        return False
    else:
...
def try_get_secret(
    ...
    **kwargs: dict[str, Any],
    ...
    Raises:
        DataRheoInputError: If an invalid source name is provided in the `sources` argument.
    """
    with suppress(exc.DataRheoSecretNotFoundError):
        return get_secret(
            secret_name,
            sources=sources,
            allow_prompt=False,
...
def get_secret(
    ...
    **kwargs: dict[str, Any],
    ...
    Raises:
        DataRheoSecretNotFoundError: If the secret is not found in any of the configured sources,
    ...
    raise exc.DataRheoSecretNotFoundError(
