...
from datarheo.caches.util import get_default_cache
from datarheo.datasets._lazy import LazyDataset
from datarheo.progress import ProgressStyle, ProgressTracker
...
if TYPE_CHECKING:
    ...
    from datarheo.callbacks import ConfigChangeCallback
...
class Source(ConnectorBase):  # noqa: PLR0904
    ...
    def set_cursor_key(
        ...
        - Stream names are not validated by PyDataRheo. If the stream name
    ...
    def select_streams(self, streams: str | list[str]) -> None:
        ...
        for stream in streams:
            if stream not in available_streams:
                raise exc.DataRheoStreamNotFoundError(
                    stream_name=stream,
        ...
        with as_temp_files([self._hydrated_config]) as [config_file]:
            for msg in self._execute(["discover", "--config", config_file]):
                if msg.type == Type.CATALOG and msg.catalog:
                    return msg.catalog
            raise exc.DataRheoConnectorMissingCatalogError(
                connector_name=self.name,
    ...
    def _get_spec(self, *, force_refresh: bool = False) -> ConnectorSpecification:
        ...
        * Make sure the subprocess is killed when the function returns.
        ...
        raise exc.DataRheoConnectorMissingSpecError(
            connector_name=self.name,
            log_text=self._last_log_messages,
        )
    ...
    def get_stream_json_schema(self, stream_name: str) -> dict[str, Any]:
        ...
        if len(found) > 1:
            raise exc.DataRheoInternalError(
    ...
    def _get_datarheo_message_iterator(
        self,
        *,
        streams: Literal["*"] | list[str] | None = None,
    ...
    def _peek_datarheo_message(
        ...
        *,
        ...
        Raises:
            DataRheoConnectorFailedError: If a TRACE message of type ERROR is emitted.
        """
        super()._peek_datarheo_message(message, raise_on_error=raise_on_error)
    ...
    def _log_incremental_streams(
        ...
        *,
        ...
        log_message = (
            ...
            "To perform a full refresh, set 'force_full_refresh=True' in 'datarheo.read()' method."
        )
    ...
    def read(
        ...
        *,
        ...
        if not self._selected_stream_names:
            raise exc.DataRheoNoStreamsSelectedError(
                connector_name=self.name,
                available_streams=self.get_available_streams(),
            )
        ...
        except exc.DataRheoInternalError as ex:
            progress_tracker.log_failure(exception=ex)
            raise exc.DataRheoConnectorFailedError(
                connector_name=self.name,
                log_text=self._last_log_messages,
            ) from ex
        except Exception as ex:
    ...
    def _read_to_cache(  # noqa: PLR0913  # Too many arguments
        ...
        *,
        ...
        if write_strategy == WriteStrategy.REPLACE and not force_full_refresh:
            warnings.warn(
                message=(
                    ...
                    'category="datarheo.warnings.DataRheoDataLossWarning")`'
                ),
                category=exc.DataRheoDataLossWarning,
                stacklevel=1,
            )
        ...
        datarheo_message_iterator = AirbyteMessageIterator(
            self._read_with_catalog(
        ...
        cache._write_datarheo_message_stream(  # noqa: SLF001  # Non-public API
            stdin=datarheo_message_iterator,
