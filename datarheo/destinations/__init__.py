...
my_destination = dr.get_destination(
    "destination-foo",
...
takes either a `datarheo.Source` or `datarheo.ReadResult` object.
...
runtime does not support docker, you may want to use the `datarheo.caches` module to load data to
a SQL cache. Caches are mostly identical to destinations in behavior, and are implemented internally
to PyDataRheo so they can run anywhere that PyDataRheo can run.
"""
...
from datarheo.destinations.util import (
    get_destination,
...
if TYPE_CHECKING:
    ...
    from datarheo.destinations import util
