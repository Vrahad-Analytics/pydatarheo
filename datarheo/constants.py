...
"""Constants shared across the PyDataRheo codebase."""
...
def _apply_legacy_env_aliases() -> None:
    ...
    for key, value in list(os.environ.items()):
        if key.startswith(_LEGACY_ENV_PREFIX):
            os.environ.setdefault(ENV_PREFIX + key[len(_LEGACY_ENV_PREFIX) :], value)
...
DR_EXTRACTED_AT_COLUMN = "_datarheo_extracted_at"
"""A column that stores the timestamp when the record was extracted."""

DR_META_COLUMN = "_datarheo_meta"
...
`emitted_at` (`_datarheo_extracted_at`) timestamps.
"""
...
"""A set of internal columns that are reserved for PyDataRheo's internal use."""
...
DEFAULT_PROJECT_DIR: Path = _try_create_dir_if_missing(
    Path(os.getenv("DATARHEO_PROJECT_DIR", "") or Path.cwd()).expanduser().absolute(),
    desc="project",
)
"""Default project directory.

Can be overridden by setting the `DATARHEO_PROJECT_DIR` environment variable.
...
If a path is specified that does not yet exist, PyDataRheo will attempt to create it.
"""


DEFAULT_INSTALL_DIR: Path = _try_create_dir_if_missing(
    Path(os.getenv("DATARHEO_INSTALL_DIR", "") or DEFAULT_PROJECT_DIR).expanduser().absolute(),
    desc="install",
)
"""Default install directory for connectors.

If not set, defaults to `DEFAULT_PROJECT_DIR` (`DATARHEO_PROJECT_DIR` env var) or the current
...
DEFAULT_CACHE_ROOT: Path = (
    (Path(os.getenv("DATARHEO_CACHE_ROOT", "") or (DEFAULT_PROJECT_DIR / ".cache")))
    .expanduser()
    .absolute()
)
"""Default cache root is `.cache` in the current working directory.

The default location can be overridden by setting the `DATARHEO_CACHE_ROOT` environment variable.
...
your mounted Google Drive by setting this to a path like `/content/drive/MyDrive/DataRheo/cache`.
"""

DEFAULT_CACHE_SCHEMA_NAME = "datarheo_raw"
...
TEMP_DIR_OVERRIDE: Path | None = (
    Path(os.environ["DATARHEO_TEMP_DIR"]) if os.getenv("DATARHEO_TEMP_DIR") else None
)
"""The directory to use for temporary files.

This value is read from the `DATARHEO_TEMP_DIR` environment variable. If the variable is not set,
...
TEMP_FILE_CLEANUP = _str_to_bool(
    os.getenv(key="DATARHEO_TEMP_FILE_CLEANUP"),
    default=True,
)
"""Whether to clean up temporary files after use.

This value is read from the `DATARHEO_TEMP_FILE_CLEANUP` environment variable. If the variable is
not set, the default value is `True`.
"""

DATARHEO_OFFLINE_MODE = _str_to_bool(
    os.getenv(key="DATARHEO_OFFLINE_MODE"),
    default=False,
)
...
DATARHEO_PRINT_FULL_ERROR_LOGS: bool = _str_to_bool(
    os.getenv(key="DATARHEO_PRINT_FULL_ERROR_LOGS", default=os.getenv("CI")),
    default=False,
)
...
NO_UV: bool = os.getenv("DATARHEO_NO_UV", "").lower() in {"1", "true", "yes"}
"""Whether to disable uv and use pip for Python package management.

This value is determined by the `DATARHEO_NO_UV` environment variable. When `DATARHEO_NO_UV`
...
For more information, see the `datarheo.secrets` module documentation.
"""
...
MCP_READONLY_MODE_ENV_VAR: str = "DATARHEO_CLOUD_MCP_READONLY_MODE"
...
MCP_DOMAINS_DISABLED_ENV_VAR: str = "DATARHEO_MCP_DOMAINS_DISABLED"
...
MCP_DOMAINS_ENV_VAR: str = "DATARHEO_MCP_DOMAINS"
...
MCP_TRUSTED_EXECUTION_ENV_VAR: str = "DATARHEO_MCP_TRUSTED_EXECUTION"
...
MCP_WORKSPACE_ID_HEADER: str = "X-DataRheo-Workspace-Id"
...
MCP_ORGANIZATION_ID_HEADER: str = "X-DataRheo-Organization-Id"
...
Enable them with `DATARHEO_MCP_INSIDERS` / `X-MCP-Insiders`, or by naming the module in
the include list.
"""

MCP_INSIDERS_ENV_VAR: str = "DATARHEO_MCP_INSIDERS"
...
MCP_CONFIG_READONLY_MODE: str = "datarheo_readonly_mode"
"""Config arg name for the legacy DATARHEO_CLOUD_MCP_READONLY_MODE setting."""

MCP_CONFIG_EXCLUDE_MODULES: str = "datarheo_exclude_modules"
"""Config arg name for the legacy DATARHEO_MCP_DOMAINS_DISABLED setting."""

MCP_CONFIG_INCLUDE_MODULES: str = "datarheo_include_modules"
"""Config arg name for the legacy DATARHEO_MCP_DOMAINS setting."""
