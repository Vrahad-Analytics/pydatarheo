...
connectors. It is used by both the CLI (`pydr destination-smoke-test`) and the
...
def _extract_trace_error_from_log(ex: Exception) -> str | None:
    ...
    PyDataRheo only captures the `message` in the exception chain, so this
...
def _sanitize_error(ex: Exception) -> str:
    ...
    Uses `get_message()` when available (PyDataRheo exceptions) to avoid
...
def _build_preflight_scenario() -> dict[str, Any]:
    ...
    from `datarheo.cli.smoke_test_source._scenarios`.
    """
    return {
