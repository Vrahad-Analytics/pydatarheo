...
"""Telemetry implementation for PyDataRheo.

Usage reporting is **opt-in and off by default**. Nothing is sent anywhere unless you set
`DATARHEO_TRACKING_KEY` to a Segment write key that you own. Setting `DO_NOT_TRACK` to any
value, or enabling offline mode, disables reporting outright.
...
PyDataRheo is used, and to dedupe users to determine how many users or use cases there are.


Here is what is tracked:
- The version of PyDataRheo.
- The Python version.
- The OS.
...
- Flags to help us understand if PyDataRheo is running on CI, Google Colab, or another environment.
...
from datarheo._util import meta
from datarheo._util.connector_info import (
...
PYDATARHEO_APP_TRACKING_KEY = os.environ.get("DATARHEO_TRACKING_KEY", "")
"""Segment write key for the PyDataRheo application.

There is no default. Usage reporting is therefore *off* unless you set
`DATARHEO_TRACKING_KEY` to a write key you own, and no data leaves the machine
until you do.
"""


PYDATARHEO_SESSION_ID = str(ulid.ULID())
"""Unique identifier for the current invocation of PyDataRheo.
...
_ENV_ANALYTICS_ID = "DATARHEO_ANALYTICS_ID"  # Allows user to override the anonymous user ID
_ANALYTICS_FILE = Path.home() / ".datarheo" / "analytics.yml"
...
def _setup_analytics() -> str | bool:
    ...
    if os.environ.get(DO_NOT_TRACK) or DATARHEO_OFFLINE_MODE:
    ...
    if not _ANALYTICS_FILE.exists():
        ...
        print(
            "Thank you for using PyDataRheo!\n"
    ...
    try:
        ...
        _ANALYTICS_FILE.write_text(
            "# This file is used by PyDataRheo to track anonymous usage statistics.\n"
...
def send_telemetry(
    *,
    ...
    # Usage reporting is opt-in: with no write key configured there is nowhere to send to.
    if not PYDATARHEO_APP_TRACKING_KEY:
        return

    payload_props: dict[str, str | int | dict] = {
        "session_id": PYDATARHEO_SESSION_ID,
        "state": state,
        "version": get_version(),
    ...
    if exception:
        if isinstance(exception, exc.DataRheoCloudError):
    ...
    with suppress(Exception):
        ...
        _ = requests.post(
            ...
            auth=(PYDATARHEO_APP_TRACKING_KEY, ""),
            json={
